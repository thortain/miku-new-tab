    ...
    const WIDGET_REGISTRY = {
      ...
      rss: { name:'RSS Feed', icon:'📰', unique:true, render:async(el)=>{el.innerHTML='<div class="rss-widget"><div class="rss-row"><input class="rss-input" id="rssUrl" placeholder="https://example.com/feed.xml" maxlength=500><button class="rss-btn" id="rssGo">Go</button></div><div class="rss-output" id="rssOut">Enter a feed URL</div></div>';const out=el.querySelector('#rssOut'),urlIn=el.querySelector('#rssUrl');urlIn.value=localStorage.getItem('miku-rss-url')||'';urlIn.onkeydown=e=>{if(e.key==='Enter')el.querySelector('#rssGo').click()};el.querySelector('#rssGo').onclick=async()=>{const u=urlIn.value.trim();if(!u)return;localStorage.setItem('miku-rss-url',u);out.textContent='Loading...';try{const r=await fetch('https://api.rss2json.com/v1/api.json?rss_url='+encodeURIComponent(u));const d=await r.json();if(d.status!=='ok')throw new Error();const items=d.items.slice(0,5);out.innerHTML=items.map(i=>'<div class="rss-item"><a href="'+i.link+'" target="_blank" class="rss-title">'+i.title+'</a><div class="rss-date">'+new Date(i.pubDate).toLocaleDateString()+'</div></div>').join('');}catch{out.innerHTML='<span class="werror">Could not load feed</span>'}};if(urlIn.value)el.querySelector('#rssGo').click()}},
